...
public class HttpUtils {
    ...
    private static AndroidHttpClient createHttpClient(Context context) {
    	String userAgent = "Android-Mms/2.0"; //MmsConfig.getUserAgent();
        AndroidHttpClient client = AndroidHttpClient.newInstance(userAgent, context);
        HttpParams params = client.getParams();
        HttpProtocolParams.setContentCharset(params, "UTF-8");

        // set the socket timeout
        int soTimeout = 60*1000; //1024;// MmsConfig.getHttpSocketTimeout();

        NetLog.v("[HttpUtils] createHttpClient w/ socket timeout " + soTimeout + " ms, "
                    + ", UA=" + userAgent);
